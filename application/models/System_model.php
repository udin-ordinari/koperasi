...
class System_model extends CI_Model {
	...
	public function get_logo_values($key)
	{
		$query = $this->db->where('nama', $key)->limit(1)->get('app_logo');
		if ($query->num_rows() === 1)
		{
			$result = $query->row();
			return $result->isi;
	...
	public function tarikan_sukarela($user)
	{
		return $this->db->select_sum('jumlah', 'total')->where('user_id', $user)->get('app_penarikan');
	}
	public function total_tarikan_sukarela()
	{
		return $this->db->select_sum('jumlah', 'total')->get('app_penarikan');
	}
	public function get_list_barang()
	{
		return $this->db->get('app_barang');
	}
	public function get_satuan($id)
	{
		return $this->db->where('id', $id)->limit(1)->get('app_satuan')->row();
	}
}
