...
class Simpanan extends App_Controller {
	...
	{
		redirect('transaksi/simpanan/data');
	}
	...
	public function get_data_simpanan()
	{
		...
		foreach($hasil->result() as $row)
		{
			...
			$data[] = array(
					...
					'<a href="javascript:void(0);" class="btn btn-info btn-sm" title="Lihat Detail" data-href="'.base_url('transaksi/simpanan/detail_simpanan/'.$row->user_id).'" data-name="Detail Data" data-bs-toggle="modal" data-bs-target="#modalKing"><i class="fa fa-eye fa-fw fa-1x"></i></a>
					<a href="javascript:void(0);" class="btn btn-warning btn-sm" title="Edit" data-href="'.base_url('transaksi/simpanan/edit_simpanan/'.$row->user_id).'" data-name="Edit Data" data-bs-toggle="modal" data-bs-target="#modalKing"><i class="fa fa-pencil fa-fw fa-1x"></i></a>
					'					
				);
		}
		$result = array(
		...
		if($this->input->is_ajax_request())
		{
			if($this->validasi())
			{
				...
				if($token = session('app_token'))
				{
					...
					$query = $this->db->where('MONTH(tgl)', $tanggal)->where('YEAR(tgl)', $tgl)->where('user_id', $userid)->where('jns_simp', $simp)->where('jumlah', $jumlah)->get('app_simpanan');
	...
	public function edit_simpanan($id)
	{
		...
		$this->vars['anggota'] 	= $this->db->where('id', $id)->get('app_anggota')->row();
		$this->load->view('transaksi/simpanan/edit-simpanan-anggota', $this->vars);
	}
