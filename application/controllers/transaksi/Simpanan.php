...
class Simpanan extends App_Controller {
	...
	public function get_data_simpanan()
	{
		...
		$hasil 	= $this->db->where('is_deleted', '0')->order_by('user_id', 'asc')->group_by('user_id')->get('app_simpanan');
		$data 	= [];
		$no 	= 1;
	...
	public function add_simpanan()
	{
		if($this->input->is_ajax_request())
		{
			if($this->validasi())
			{
				...
				{		
					$tanggal = date('Y-m', strtotime($tgl_trans));
					$query = $this->db->where('user_id', $user)->where('jns_simp', $simp)->like('tgl', $tanggal)->limit(1)->get('app_simpanan');

					if($query->num_rows() == 1)
					{
						$data = $query->row();
						if($data->saldo_awal === '1')
						{
							$this->vars['status'] = 'info';
							$this->vars['title']  = 'Maaf !';
							$this->vars['message']= 'Saldo Awal '.get_jenis_simpanan($data->jns_simp)->nama.' Sudah Ada !'; 
						}
						elseif($data->saldo_awal === '0')
						{
							$this->vars['status'] = 'info';
							$this->vars['title']  = 'Maaf !';
					...
					{
						$tanggal = date('y', strtotime($tgl_trans)).''.date('m', strtotime($tgl_trans)).''.date('d', strtotime($tgl_trans));
						$dataset = array(
	...
	public function data_penutupan()
	{
		$this->vars['pagetitle']    = 'Data';
		$this->vars['title'] 	    = 'Penutupan Simpanan Anggota';
		$this->vars['content']      = 'transaksi/simpanan/data-penutupan';
		$this->load->view('templates/index', $this->vars);
	}
	public function get_data_penutupan()
	{
		$draw = intval($this->input->get('draw'));
		$start = intval($this->input->get('start'));
		$length = intval($this->input->get('length'));
		$tahun = tahun_buku()->periode;
		$hasil = $this->db->get('app_penutupan');
		$data = [];
		$no = 1;
		foreach($hasil->result() as $row)
		{
	...
	public function saldo_awal_simpanan()
	{
		$this->vars['title'] 	    = 'Saldo Awal Simpanan';
		$this->vars['content']      = 'master/simpanan/data-saldo-awal';
		$this->load->view('templates/index', $this->vars);
	}
	public function get_saldo_awal()
	{
		...
		$start = intval($this->input->get('start'));
		$length = intval($this->input->get('length'));
		$hasil = $this->db->where('saldo_awal', '1')->where('is_deleted', '0')->order_by('user_id', 'asc')->group_by('user_id')->get('app_simpanan');
		$data = [];
		$no = 1;
		foreach($hasil->result() as $row)
		{
			$pokok 		= $this->db->select_sum('jumlah', 'total')->where('user_id', $row->user_id)->where('jns_simp', '1')->where('saldo_awal', '1')->where('is_deleted', '0')->get('app_simpanan')->row();
			$wajib 		= $this->db->select_sum('jumlah', 'total')->where('user_id', $row->user_id)->where('jns_simp', '2')->where('saldo_awal', '1')->where('is_deleted', '0')->get('app_simpanan')->row();
			$swp   		= $this->db->select_sum('jumlah', 'total')->where('user_id', $row->user_id)->where('jns_simp', '4')->where('saldo_awal', '1')->where('is_deleted', '0')->get('app_simpanan')->row();
			$sukarela   	= $this->db->select_sum('jumlah', 'total')->where('user_id', $row->user_id)->where('jns_simp', '3')->where('saldo_awal', '1')->where('is_deleted', '0')->get('app_simpanan')->row();
			$suka		= $sukarela->total - tarikan_sukarela($row->user_id)->total;


			$data[] = array(
					$no++.'.',
					get_anggota($row->user_id)->nama.', '.get_cabang(get_anggota($row->user_id)->cabang)->nama,
					'Rp. '.number_format((int)$pokok->total, 0, ', ', '.'),
					'Rp. '.number_format((int)$wajib->total, 0, ', ', '.'),
					'Rp. '.number_format((int)$swp->total, 0, ', ', '.'),
					'Rp. '.number_format((int)$suka, 0, ', ', '.'),
					'Rp. '.number_format((int)$pokok->total + $wajib->total + $swp->total + $suka, 0, ', ', '.'),
					'<button class="btn btn-info btn-sm" title="Lihat Detail" data-href="'.base_url('transaksi/simpanan/detail_simpanan/'.$row->user_id).'" data-name="Detail Data" data-bs-toggle="modal" data-bs-target="#modalKing"><i class="fa fa-eye fa-fw fa-1x"></i></button>
					<button class="btn btn-warning btn-sm" title="Edit" onclick="Peringatan('.$row->user_id.');"><i class="fa fa-pencil fa-fw fa-1x"></i></button>
					'					
				);
