		...
		if($this->input->is_ajax_request())
		{
			if($this->validasi())
			{
				...
				{		
    					...
    					$query   = $this->db->where('user_id', $user)->where('jns_simp', $simp)->where('saldo_awal', $awal)->get('app_simpanan');
					
					if($query->num_rows() > 0)
					{
						$data = $query->row();
						if($data->jns_simp == $simp && $data->saldo_awal == '1')
						{
							$this->vars['status'] 	= 'info';
							$this->vars['message'] 	= 'Saldo Awal '.get_jenis_simpanan($simp)->nama.' Ada';
						}

						if(date('Y-m', strtotime($data->tgl)) == date('Y-m', strtotime($tgl_trans)))
						{
							$this->vars['status'] 	= 'info';
							$this->vars['message'] 	= get_jenis_simpanan($simp)->nama.' Bulan '.date('Y-m', strtotime($tgl_trans)).' Ada';
						}
					}
					else
					{
						...
						{
							$this->vars['status'] 	= 'info';
							$this->vars['message'] 	= 'Simpanan Pokok Sudah Ada';
						}
