	...
	<div class="card-body p-1">
		<div class="table-responsive">
			<table class="table table-striped table-bordered lookup" style="cursor:pointer">
				...
				<tbody>
				<?php 
					$hasil = $this->db->where('status !=', 'lunas')->where('status !=', 'baru')->group_by('user_id')->get('app_pinjaman')->result();
					foreach($hasil as $row)
					{
						
							$user = $this->db->where('id', $row->user_id)->get('app_anggota')->row();
							$cab  = $this->db->where('id', $user->cabang)->get('app_cabang')->row();
							$jum  = $this->db->select_sum('jumlah')->where('user_id', $row->user_id)->get('app_pinjaman')->row();
							$pokok= $row->jumlah / $row->tempo;
						
				?>
					<tr>
						<td class="sembunyi"><?php echo $row->id; ?></td>
...
$(tableBody).on('click', 'tr', function () {
	...
	$('form').find("input[name='id'][type='text']").val(data[0]);
