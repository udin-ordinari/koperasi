				...
				<div class="card-body">
					<div class="table-responsive fs-7">
						<table id="piutang_belum_terbayar" class="piutang_belum_terbayar table mb-0 align-middle">
							...
							<tbody>
<?php
$hasil = $this->db->where('YEAR(tgl) <=', $tahun)->order_by('user_id', 'asc')->get('app_pinjaman');
$total = $this->db->select_sum('jumlah')->get('app_pinjaman')->row();
...
<?php } ?>
								...
								<tr>
									...
									<th class="text-center text-secondary">Rp. <?php echo number_format((int)$total->jumlah, 0, ', ', '.'); ?></th>
									<th class="text-center text-secondary">Rp. <?php echo number_format((int)$total->jumlah, 0, ', ', '.'); ?></th>
