				...
				<div class="card">
					<div class="card-body">
						...
						<table class="table mb-0 align-middle">
							...
							<tbody>
<?php
$hasil = $this->db->order_by('user_id', 'asc')->get('app_piutang_pinjaman');
$total = $this->db->select_sum('pokok_terbayar')->get('app_piutang_pinjaman')->row();
$terima = $this->db->select_sum('bunga_terbayar')->get('app_piutang_pinjaman')->row();
$no = 1;
...
?>
								<tr>
									...
									<td class="text-primary kandel">Rp. <?php echo number_format($row->pokok_terbayar, 0, ', ', '.'); ?></td>
									<td class="text-success kandel">Rp. <?php echo number_format($row->bunga_terbayar, 0, ', ', '.'); ?></td>
									<td>-</td>
									<td class="text-danger kandel">Rp. <?php echo number_format($row->pokok_terbayar + $row->bunga_terbayar, 0, ', ', '.'); ?></td>
								</tr>
<?php } ?>
							</tbody>
							</tfoot>
								<tr>
									...
									<th class="text-left text-secondary">Rp. <?php echo number_format((int)$total->pokok_terbayar, 0, ', ', '.'); ?></th>
									<th class="text-left text-secondary">Rp. <?php echo number_format((int)$terima->bunga_terbayar, 0, ', ', '.'); ?></th>
									<th class="text-center">-</th>
									<th class="text-left text-secondary kandel">Rp. <?php echo number_format((int)$total->pokok_terbayar + $terima->bunga_terbayar, 0, ', ', '.'); ?></th>
								</tr>
