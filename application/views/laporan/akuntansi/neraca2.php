...
<?php
$aset = $this->db->where('tampil', 'neraca-usp')->get('app_akuntansi');
$no = 1;
...
<?php if($row->id == '1') { ?>									
										<td class="text-right">Rp. <?php echo number_format(get_saldo_awal($row->id, '01', $tahun-1), 0, ', ','.'); ?></td>
										<td class="text-right">Rp. <?php echo number_format((int)get_trans_akun($row->id, '01', $tahun - 1) + total_kas_usp($tahun), 0, ', ','.'); ?></td>
<?php } if($row->id == '2') { ?>
										<td class="text-right">Rp. <?php echo number_format((int)$this->db->select_sum('jumlah', 'total')->get('app_pinjaman')->row()->total, 0, ', ','.'); ?></td>
...
<?php if($r->id == '15') { ?>									
										<td class="text-right">Rp. <?php echo number_format((int)get_trans_akun('15', '01', $tahun - 1), 0, ', ','.'); ?></td>
...
<?php
$aset = $this->db->where('jenis', '01')->where('kelompok', '02')->where('tampil', 'neraca')->get('app_akun');
$no = 1;
foreach($aset->result() as $row) {	
?>
									<tr>
