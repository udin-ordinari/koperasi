...
<?php
$aset = $this->db->where('jenis', '01')->where('tampil', 'neraca-usp')->get('app_akun');
$no = 1;
...
<?php } if($row->id == '2') { ?>
										<td class="text-right">Rp. <?php echo number_format(get_trans_akun($row->id, $tahun - 1), 0, ', ','.'); ?></td>
										<td class="text-right">Rp. <?php echo number_format((int)get_trans_akun($row->id, $tahun - 1) + total_pinjaman($tahun-1) + total_pinjaman($tahun), 0, ', ', '.'); ?></td>
<?php } if($row->id == '3') { ?>
										<td class="text-right">Rp. <?php echo number_format(get_trans_akun($row->id, $tahun - 1), 0, ', ','.'); ?></td>
...
<?php
$aset = $this->db->where('jenis', '4')->where('tampil', 'neraca-usp')->get('app_akun');
$n = 1;
...
<?php if($r->id == '6') { ?>									
										<td class="text-right">Rp. <?php echo number_format((int)total_simpanan_sukarela($tahun-1), 0, ', ','.'); ?></td>
...
<?php
$aset = $this->db->where('jenis', '5')->where('tampil', 'neraca-usp')->get('app_akun');
$n = 1;
...
<?php if($ro->id == '8') { ?>									
										<td class="text-right">Rp. <?php echo number_format((int)total_simpanan_lain($tahun-1), 0, ', ','.'); ?></td>
										<td class="text-right">Rp. <?php echo number_format((int)total_simpanan_lain($tahun), 0, ', ','.'); ?></td>
<?php } if($ro->id == '9') { ?>
										<td class="text-right">Rp. <?php echo number_format((int)get_trans_akun($ro->id, $tahun - 1), 0, ', ','.'); ?></td>
										<td class="text-right">Rp. <?php echo number_format((int)modal_tidak_tetap($tahun - 1) + modal_tidak_tetap($tahun), 0, ', ', '.'); ?></td>
<?php } if($ro->id == '10') { ?>
										<td class="text-right">Rp. <?php echo number_format((int)get_trans_akun($ro->id, $tahun - 1), 0, ', ','.'); ?></td>
										<td class="text-right">Rp. <?php echo number_format((int)modal_tidak_tetap($tahun - 1) + modal_tidak_tetap($tahun), 0, ', ', '.'); ?></td>
<?php } ?>
									...
									<tr>
										<td></td>
										<td class="text-right tx-semibold">sub jumlah</td>
										<td>Rp. <?php echo number_format((int)total_simpanan_lain($tahun-1) + get_trans_akun('9', $tahun - 1) + get_trans_akun('10', $tahun - 1), 0, ', ','.'); ?></td>
										<td>Rp. </td>
									</tr>
									<tr class="tx-14 kandel">
										<td>IV</td>
...
<?php
$aset = $this->db->where('jenis', '6')->where('tampil', 'neraca-usp')->get('app_akun');
$n = 1;
...
<?php if($ros->id == '11') { ?>									
										<td class="text-right">Rp. <?php echo number_format((int)get_trans_akun($ros->id, $tahun - 1), 0, ', ','.'); ?></td>
							...
							<table class="table text-nowrap text-md-nowrap table-hover tx-14 mb-0">
								<thead class="table-secondary">
									<tr>
										...
										<th scope="col" class="text-start text-capitalize">Rp. <?php echo number_format(get_trans_akun('1', $tahun - 1) + get_trans_akun('2', $tahun - 1) + get_trans_akun('3', $tahun - 1) + get_trans_akun('4', $tahun - 1) + get_trans_akun('5', $tahun - 1), 0, ', ','.'); ?></th>
										<th scope="col" class="text-start text-capitalize">Rp. <?php echo number_format($this->transaksi->total_kas_usp($tahun) + get_trans_akun('2', $tahun - 1) + total_pinjaman($tahun-1) + total_pinjaman($tahun) + get_trans_akun('4', $tahun - 1) + get_trans_akun('5', $tahun), 0, ', ','.'); ?></th>
...
<?php
$aset = $this->db->where('jenis', '1')->where('tampil', 'neraca-induk')->get('app_akun');
$no = 1;
foreach($aset->result() as $row) {	
?>
									<tr>
