...
function SelectChanged()
{
	...
	{
		$.ajax({
			url: "<?php echo base_url('laporan/pembantu/get_buku_kas/');?>" + bulan,
			data: {'bulan':bulan},
			type: 'POST',
			async: false,
			success: function(response)
			{
				...
				$.ajax({
					url: "<?php echo base_url('laporan/pembantu/get_detail_buku_kas/');?>" + response.bln,
					type: 'GET',
					async: false,
					...
					error: function(res)
					{
						Swal.fire({
							title: "Maaf !",
							html: "Sistem Sedang Tidak Baik-baik Saja !",
							icon: "warning"
						});
						$('#metune').addClass('sembunyi');
					}
			...
			error: function(response)
			{
				Swal.fire({
					title: "Maaf !",
					html: "Sistem Sedang<br>Tidak Baik-baik Saja !",
					imageUrl: "<?php echo base_url('assets/img/down.webp'); ?>",
					imageWidth: 120,
					imageHeight: 120,
					imageAlt: "Sistem Down"
				});
			}
		});
	}
