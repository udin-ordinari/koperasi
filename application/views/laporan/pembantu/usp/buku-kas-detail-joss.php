					...
					<table id="" class="table table-bordered mb-0 fs-7">
						<thead class="table-secondary text-center">
							...
							<tr class="table-info">
								<td></td>
								<td></td>
								<td class="kandel" style="text-align:left">Saldo Awal <?php echo bulan($bulan); ?> <?php echo $tahun; ?></td>
								<td colspan="2">
<?php
...
?>	
						...
						<tbody>			
							<tr>
						 	...
						 	<?php
							$no 	= 1;
							$trans 	= $this->db->where('MONTH(tgl)', $bulan)->where('YEAR(tgl)', $tahun)->get('app_transaksi');
							foreach ($trans->result() as $row) {
								$debet 	= $this->db->select("SUM(jumlah) as total")->where('MONTH(tgl)', $bulan)->where('YEAR(tgl)', $tahun)->where('jenis', 'debet')->get('app_transaksi')->row();
								$kredit = $this->db->select("SUM(jumlah) as total")->where('MONTH(tgl)', $bulan)->where('YEAR(tgl)', $tahun)->where('jenis', 'kredit')->get('app_transaksi')->row();
							?>	
							<tr>
								<td><?php echo $no++; ?> .</td>
							...
							<tr>
						 	...
						 	<?php
								$simp  = $this->db->where('MONTH(tgl)', $bulan)->where('YEAR(tgl)', $tahun)->get('app_simpanan');
							...
							<?php
								$pinj  = $this->db->where('MONTH(tgl)', $bulan)->where('YEAR(tgl)', $tahun)->where('status', 'disetujui')->get('app_pinjaman');
								foreach ($pinj->result() as $ro)
								{ 
									$totpinjaman = $this->db->select("SUM(total) as total")->where('MONTH(tgl)', $bulan)->where('YEAR(tgl)', $tahun)->where('status', 'disetujui')->get('app_pinjaman')->row();
							?>	
							...
							<?php }
								$angs  = $this->db->where('MONTH(tgl)', $bulan)->where('YEAR(tgl)', $tahun)->get('app_angsuran');
								foreach ($angs->result() as $r)
								{ 
									$totangsuran = $this->db->select("SUM(jumlah) as total")->where('MONTH(tgl)', $bulan)->where('YEAR(tgl)', $tahun)->get('app_angsuran')->row();								
							...
							<?php } ?>
							<?php 
								$pena  = $this->db->where('MONTH(tgl)', $bulan)->where('YEAR(tgl)', $tahun)->get('app_penarikan');
								foreach ($pena->result() as $rod)
								{ 
									$totpenarikan = $this->db->select("SUM(jumlah) as total")->where('MONTH(tgl)', $bulan)->where('YEAR(tgl)', $tahun)->get('app_penarikan')->row();
							?>	
							<tr>
								<td><?php echo $no++; ?> .</td>
							...
							<?php 
								$untungswp  = $this->db->where('MONTH(tgl)', $bulan)->where('YEAR(tgl)', $tahun)->group_start()->where('status', 'disetujui')->or_where('status', 'lunas')->group_end()->get('app_pinjaman');
								foreach ($untungswp->result() as $rode)
								{ 
									$totswp = $this->db->select("SUM(swp) as total")->where('MONTH(tgl)', $bulan)->where('YEAR(tgl)', $tahun)->group_start()->where('status', 'disetujui')->or_where('status', 'lunas')->group_end()->get('app_pinjaman')->row();
							?>	
							<tr>
								<td><?php echo $no++; ?> .</td>
							...
							<?php 
								$untungrsk  = $this->db->where('MONTH(tgl)', $bulan)->where('YEAR(tgl)', $tahun)->group_start()->where('status', 'disetujui')->or_where('status', 'lunas')->group_end()->get('app_pinjaman');
								foreach ($untungrsk->result() as $roden)
								{ 
									$totrsk = $this->db->select("SUM(rsk) as total")->where('MONTH(tgl)', $bulan)->where('YEAR(tgl)', $tahun)->group_start()->where('status', 'disetujui')->or_where('status', 'lunas')->group_end()->get('app_pinjaman')->row();
							?>	
							<tr>
								<td><?php echo $no++; ?> .</td>
...
?>
							<tr>
								...
								<td class="text-right kandel fs-6">Rp. <?php echo number_format($melbet, 0,',','.'); ?></td>
								<td class="text-right kandel fs-6">Rp. <?php echo number_format($medal, 0,',','.'); ?></td>
								<td class="text-right kandel fs-6">Rp. <?php echo number_format($nongol, 0,',','.'); ?></td>
							</tr>
						</tfoot>
					</table>
				</div>
