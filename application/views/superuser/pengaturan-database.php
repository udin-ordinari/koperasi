				...
				<div class="col-lg-6 col-md-12">
					<div class="card custom-card">
						<div class="card-body">
							...
							<p class="mg-b-10 pt-4"><div class="card custom-card card-body tx-white-8 bg-warning border-0">Setelah anda download backup database mohon untuk tabel "app_sessions" dikolom yang berisi seperti berikut <code class="tx-white kandel text-center">INSERT INTO `app_sessions` ..... bla bla</code> dihapus semua. agar tidak terjadi error saat nanti akan direstore databasenya.</div></p>
						</div>
					</div>
				</div>
				<div class="col-lg-6 col-md-12">
					<div class="card custom-card">
						<div class="card-body">
							...
							<div class="row row-sm">
								<div class="col-sm-12 col-md-12 col-lg-12">
									...
									<div class="input-group file-browser">
										<input type="text" class="form-control border-end-0 browse-file" id="databasenya" name="databasenya" placeholder="Silahkan Pilih" readonly>
										<label class="input-group-btn">
											<span class="btn btn-primary cursor-pointer">
												Pilih <input type="file" class="form-control" id="databasenya" name="databasenya" style="display: none;">
											</span>
										</label>
									</div>
							...
							<div class="card custom-card card-body tx-white bg-secondary border-0 text-center"><span class="kandel"><i class="fa-solid fa-hand-point-left fa-2xl mr-3"></i> Ingat !</span><br>Untuk mengikuti petunjuk yang sudah tertera dalam kolom sebelah kiri berwarna oranye. mohon untuk dilakukan.</div>
						</div>
					</div>
				</div>
			</div>
...
function BackUp()
{
	...
	Notiflix.Confirm.show(
		...
		'Siap','Tidak', () => {
			...
			setTimeout(function()
			{
				location.href = "<?php echo base_url('database/backup_database');?>";
			}, 2500);
		},
		() => {
			$(".backup").removeAttr('disabled');
...
$('form').on('submit',function(e){
	...
	$.ajax({
		url: "<?php echo base_url('database/restoredb');?>",
		type: "post",
		...
		success: function(response)
		{
			...
			{
				Lobibox.notify(response.status, {title: response.title, delayIndicator: false, position: 'top right', icon: true, msg: response.message});
		...
		error: function (jqXHR, textStatus, errorMessage)
		{
			$('.Proses').removeClass('disabled');
			$('.Proses').html('Restore <i class="fa-regular fa-database fa-fw fa-lg ml-2 me-2 mr-0"></i>');
			Lobibox.notify(textStatus, {title: 'Waduh !', delayIndicator: false, position: 'top right', icon: true, msg: errorMessage});
		},
	});
});
