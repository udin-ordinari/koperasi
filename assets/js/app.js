! function() {
	...
	function b() {
		...
		Array.from(n).forEach(function(e, t) {
			t + 1 === a && (s = e), a < t + 1 && (o += e.outerHTML, e.remove()), t + 1 === n.length && s.insertAdjacentHTML
		})
	}
	...
	function H() {
		const tabs = document.querySelectorAll("#notificationItemsTabContent .tab-pane");
		tabs.forEach(tab => {
    			const notifications = tab.querySelectorAll(".notification-item");
    			const viewAll = tab.querySelector(".view-all");
    			const emptyElem = tab.querySelector(".empty-notification-elem");
        				...
        				const emptyHTML = `
          					<div class="empty-notification-elem">
            						<div class="w-25 w-sm-50 pt-3 mx-auto">
              							<img src="${_BASE_URL}/assets/img/bell.svg" class="img-fluid" alt="user-pic">
            						</div>
            						<div class="text-center pb-5 mt-2">
              							<h6 class="fs-18 fw-semibold lh-base">Hey! You have no notifications</h6>
            						</div>
          					</div>`;
        				tab.insertAdjacentHTML("beforeend", emptyHTML);
				}
			}
		});
	}
